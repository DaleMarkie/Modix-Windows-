...
export default function PortCheck() {
  ...
  const handleAction = async (name: string, action: "restart" | "stop") => {
    try {
      const res = await fetch(`http://localhost:8000/containers/${name}/${action}`, {
        method: "POST",
      });
      if (!res.ok) throw new Error(`Failed to ${action} container`);
      await loadContainers();
    } catch (err) {
      alert(`Failed to ${action} container ${name}`);
      console.error(err);
    }
  };
  ...
  const checkPort = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/check-port", {
        method: "POST",
